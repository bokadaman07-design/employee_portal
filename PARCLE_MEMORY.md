...
- The `Dashboard` view renders all monetary values (net payroll card and the monthly payroll table) in Dollars using the `$` symbol; salary amounts are stored/returned as plain numbers and the currency symbol is applied only at the presentation layer.
